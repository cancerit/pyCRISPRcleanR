...
#------------------------------Analysis methods---------------------------------
    ...
    def process_segments(cbs_dict, ignoredGenes, min_genes,controls,no_rep):
      """
      ...
      for chr,(cnarr,segrows) in cbs_dict.items():
          ...
          revertedCounts=cnarr.iloc[:,cnarr.columns.get_loc('endp')+controls+1:cnarr.columns.get_loc('avgFC')]
          nGeneInSeg = 0
          for segment in segrows.itertuples():
              ...
              nGeneInSeg = len(set(includedGenes) - set(ignoredGenes))
